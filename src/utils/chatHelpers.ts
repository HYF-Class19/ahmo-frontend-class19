...
export const getReceivers = (myId: number, members?: IMember[]) => {
  return (
    members
      ?.filter((member: IMember) => member.user.id !== myId)
      .map((member: IMember) => member.user.id) || []
  );
};

export const getDirectName = (myId: number, members?: IMember[]) => {
  if (!members) return;
  return members.find((member) => member.user.id !== myId)!.user;
};

export const isAvatarUnvisible = (
  myId: number,
  message: IMessage,
  messages: IMessage[],
  idx: number
...
export const getLastMessage = (
  lastMessage: IMessage | null = null,
  myId?: number
) => {
  if (!lastMessage) {
    return "";
